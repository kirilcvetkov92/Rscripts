...
meanvec1 <-c(1000000,50000,0)
meanvec2 <- c(2500000,150000,0)
meanvec3 <- c(3500000,300000,0)
#meanvec1 <-c(500000,50000,0)
#meanvec2 <- c(1000000,150000,0)
#meanvec3 <- c(4000000,300000,0)
...
final <- foreach(i = 1:iter, .combine=rbind, .packages = c("PoisNor")) %dopar% {
  ...
  if(dists[i]<probs[1]){
    mydata=data.frame(sm="basic",as.data.frame(t(genPoisNor(n=2, no.norm=3, no.pois=3, cmat.star=cstar1, lamvec1, sd.vec=sdvec1, mean.vec=meanvec1)[1,])))
    #sm <- "light"
  } else if(dists[i]<probs[2]){
    mydata=data.frame(sm="premium",as.data.frame(t(genPoisNor(n=2, no.norm=3, no.pois=3, cmat.star=cstar2, lamvec2, sd.vec=sdvec2, mean.vec=meanvec2)[1,])))
    #sm <- "standard"
  } else {
   mydata=data.frame(sm="advanced",as.data.frame(t(genPoisNor(n=2, no.norm=3, no.pois=3, cmat.star=cstar3, lamvec3, sd.vec=sdvec3, mean.vec=meanvec3)[1,])))
    #sm <- "plus"
  }
}
stopCluster(cl)
...
clean_data <- final
clean_data$Nr.CCY <- clean_data$Nr.CCY + 1 #at least one CCY
write.csv(clean_data,"COST\\clean_data.csv")
